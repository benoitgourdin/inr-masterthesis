...
def main():
    ...
    ds_loader = data_generation.create_data_loader(params, data_generation.PhaseType.INF, True, 1.0)
    # During inference we rely on image size stored in with the model (it's read later)
    net = train.create_model(params, torch.ones(3, dtype=torch.float32), dropout_p, activation, hidden_layers, encoding)
    ...
    for i, batch in enumerate(ds_loader):
        ...
        if task_type == config_io.TaskType.AD:
            labels_gt = batch['labels'].to(device)
            labels_gt_sparse = labels_gt[:num_points]
            ...
            train.optimize_latents(
                net, latents_batch, labels_gt_sparse[:, :num_points, :, :], coords[:, :num_points, :, :, :], latent_lr, lat_reg_lambda,
            ...
            labels_pred = impl_utils.sample_latents(latents_batch, net, coords)
        ...
        coords_img = coords.cpu().squeeze()[labels_pred.cpu().squeeze()].numpy()
        coords_img_low = coords.cpu()[:, :num_points, :, :, :].squeeze()[labels_gt.cpu()[:, :, :num_points, :, :].squeeze()].numpy()
        coords_img_high = coords.cpu().squeeze()[labels_gt.cpu().squeeze()].numpy()

        with open("/home/mil/gourdin/inr_3d_data/data/reconstruction", 'w') as f:
            for point in coords_img:
                f.write(f"{point[0]} {point[1]} {point[2]}\n")

        with open("/home/mil/gourdin/inr_3d_data/data/latent_shape", 'w') as f:
            for point in coords_img_low:
                f.write(f"{point[0]} {point[1]} {point[2]}\n")

        with open("/home/mil/gourdin/inr_3d_data/data/gt", 'w') as f:
            for point in coords_img_high:
                f.write(f"{point[0]} {point[1]} {point[2]}\n")
        ...
        spacings_cpu = np.array([[1.0, 1.0, 1.0]])
