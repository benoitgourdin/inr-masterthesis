...
class OccupancyPredictor(torch.nn.Module):
    def __init__(self, latent_dim: int, spatial_dim: int, num_layers: int,
                 layers_with_coords: List[int], dropout_p: float,  activation: str,
                 hidden_layers: int):
        ...
        in_channels[0] = latent_dim + spatial_dim
...
class AutoDecoder(torch.nn.Module):
    ...
    def __init__(self, lat_dim: int, spatial_dim: int, image_size: torch.Tensor,
                 occnet_num_layers: int, occnet_layers_with_coords: List[int], dropout_p: float,  activation: str,
                 hidden_layers: int, encoding):
        """
        ...
        if encoding != None:
            ...
            spatial_dim = 2 * spatial_dim * frequencies
            self.latent_coords = self.latent_coords.repeat(2 * frequencies)
        self.occp_pred = OccupancyPredictor(lat_dim, spatial_dim, occnet_num_layers,
                                            occnet_layers_with_coords, dropout_p, activation,
                                            hidden_layers)
    ...
    def forward(self, latents: torch.Tensor, coordinates: torch.Tensor) -> torch.Tensor:
        ...
        if self.encode:
            coordinates = self.encoding(coordinates)
