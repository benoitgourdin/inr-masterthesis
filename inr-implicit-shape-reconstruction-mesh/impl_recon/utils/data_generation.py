...
def load_volumes(volumes_dir: Path, casenames: List[str]) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
    """Load axis-aligned volumes."""
    volumes: list[np.ndarray] = []
    occupancy: list[np.ndarray] = []
    for casename in casenames:

        # curr_pattern = f'{casename}*.stl*'
        curr_pattern = f'{casename}*.pkl*'
        ...
        # io_utils.save_pkl_file(files[0])
        # coords, labels = io_utils.load_stl_file(files[0])
        coords, labels = io_utils.load_pkl_file(files[0])
        volumes.append(coords)
        occupancy.append(labels)
        # print("loaded: " + casename)
    # Convert everything to float32 and/or tensors
    volumes_tnsr = [torch.from_numpy(x).to(torch.float32) for x in volumes]
    labels_tnsr = [torch.from_numpy(x).to(torch.float32) for x in occupancy]
    return volumes_tnsr, labels_tnsr
...
class OrthogonalSlices(data.Dataset):
    def __init__(self, labels_dir: Path, casenames: List[str], verbose: bool):
        self.casenames = casenames
        self.coords, self.labels = load_volumes(labels_dir, self.casenames)
        size_min = 100000000000
...
class ImplicitDataset(OrthogonalSlices):
    ...
    """
    def __init__(self, labels_dir: Path, casenames: List[str],
                 val_fraction: float,  num_points_per_example: int, slice_step_size: int,
        ...
        :param num_points_per_example: number of random points (per dim) per sparse volume
        ...
        :param verbose:
        """
        self.size_min = super().__init__(labels_dir, casenames, verbose)
        ...
        size = int(round(self.coords[0].size()[0] ** (1./3)))
        self.image_size = torch.from_numpy(np.array([size, size, size]))
        ...
        if phase_type != PhaseType.INF:
            ...
            if phase_type == PhaseType.VAL:
                ...
                self.coords = [self.coords[i] for i in val_caseids]
        ...
        self.num_points_per_example = num_points_per_example \
            if num_points_per_example > 0 else -1
    ...
    def __getitem__(self, item: int):
        ...
        if self.num_points_per_example > 0:  # use random subsample from sparse volume
            ...
            indices = torch.randperm(coords.size()[0])[:int(self.num_points_per_example * self.size_min)] #coords.size()[0]
        else:  # use full sparse volume
            # Generate a 3D dense volume [SX, SY, SZ, 3]
            indices = torch.randperm(coords.size()[0])
        coords = coords[indices].unsqueeze(1).unsqueeze(1)
        label_values = label_values[indices].unsqueeze(1)#.unsqueeze(1)
...
def create_data_loader(params: Dict, phase_type: PhaseType, verbose: bool, num_points_per_example: float) -> data.DataLoader:
    """For AD, use shared training and validation during train/val, and not during inference.
    ...
    if task_type == config_io.TaskType.AD:
        if 'sample_orthogonal_slices' in params and params['sample_orthogonal_slices']:
            ...
            ds = ImplicitDataset(
                labels_dir, casenames, val_fraction, num_points_per_example,
